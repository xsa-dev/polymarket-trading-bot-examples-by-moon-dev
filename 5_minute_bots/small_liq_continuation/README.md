...
strong cascades at 50–85c). This bot hunts the OTHER pocket the logs found: the smaller
liquidation tier, where the continuation side is still cheap — 30–45 cents — because the
...
- **Liq-signal fills at 0.30–0.40: 48.8% win at a 34.1c average → +43% EV** (n=41) —
  the best cell in the entire order-flow dataset.
- Control group, same mechanic with MACD/CVD signals: 18–31% win → **-8% to -48% EV**.
...
- Continuation-side ask **0.30–0.45 only**, **60–240 seconds** left, taker. One entry
  per window.
...
and will pay 1–3c more, so the realistic live edge is lower, maybe much lower. The $25K
